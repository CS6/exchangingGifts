...
for (var tested = 0; tested < 1; tested++) {
  ...
  for (let user = 0; user < giftsFromUser.length; user++) {
    const gifts = giftsFromUser[user];

    while (gifts.some((gift) => gift.user === user)) {
      let exchangeableUser;

      do {
        exchangeableUser = Math.floor(Math.random() * giftsFromUser.length);
      } while (exchangeableUser === user || exchangeableUser === gifts[0].user);

      giftsFromUser[exchangeableUser].push(giftsFromUser[user].shift());
      giftsFromUser[user].push(giftsFromUser[exchangeableUser].shift());
    }
  }

  var usersWhoStillHoldSelfGift = Object.keys(giftsFromUser)
    .map(Number)
    .filter((user) => giftsFromUser[user].some((gift) => gift.user === user));
